...
class FlacDecoder final : public IAudioDecoder {
  public:
    ...
    bool open(const std::string &path) override {
        ...
        if (!dec_) {
            logToDebugScreen("FLAC: alloc failed");
            return false;
        }
        ...
        FLAC__StreamDecoderInitStatus st = FLAC__stream_decoder_init_file(
            dec_, path.c_str(), writeCallback, metadataCallback, errorCallback, this);
    ...
    int decode(int16_t *buffer, int maxFrames) override {
        ...
        while ((int) pcmReady() < maxFrames && !eof_) {
            ...
            if (FLAC__stream_decoder_get_state(dec_) == FLAC__STREAM_DECODER_END_OF_STREAM) {
                eof_ = true;
            }
        }
    ...
    double getPositionSeconds() const override {
        return (sampleRate_ > 0) ? (double) currentSample_ / (double) sampleRate_ : 0.0;
    }

    void seekTo(double seconds) override {
        if (!dec_ || sampleRate_ <= 0) {
            return;
        }
  ...
  private:
    ...
    static FLAC__StreamDecoderWriteStatus writeCallback(const FLAC__StreamDecoder * /*dec*/,
        ...
        auto toInt16 = [&](FLAC__int32 s) -> int16_t {
            if (bps > 16) {
                return (int16_t) (s >> (bps - 16));
            }
            if (bps < 16) {
                return (int16_t) (s << (16 - bps));
            }
            return (int16_t) s;
        };
    ...
    static void metadataCallback(const FLAC__StreamDecoder * /*dec*/,
        ...
        if (metadata->type == FLAC__METADATA_TYPE_PICTURE && self->coverArtBytes_.empty()) {
            const auto &pic = metadata->data.picture;
            if (pic.data && pic.data_length > 0) {
                self->coverArtBytes_.assign(reinterpret_cast<const char *>(pic.data),
                                            pic.data_length);
            }
        }
