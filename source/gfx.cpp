    ...
    const std::string& nowPlayingTrack)
{
    ...
    while (y < MAX_Y) {
        if (!drawnHistNowSep && virtIdx == 0 && topVirtualIdx < 0) {
            ...
            C2D_DrawRectSolid(START_X + 4.0f, ly, 0.52f,
                              400.0f - START_X - 8.0f, 1.0f,
        ...
        if (virtIdx < -histSz || virtIdx > qSz + aSz) break;

        const bool isSelected = (virtIdx == selectedVirtualIdx);

        if (virtIdx == 0) {
            if (y + CARD_H > MAX_Y) break;

            if (isSelected)
                C2D_DrawRectSolid(START_X, y - 1.0f, 0.4f,
                                  400.0f - START_X, CARD_H + 1.0f,
                                  C2D_Color32(0x1A, 0x2E, 0x1A, 0xFF));
            else
                C2D_DrawRectSolid(START_X, y - 1.0f, 0.38f,
                                  400.0f - START_X, CARD_H + 1.0f,
                                  C2D_Color32(0x16, 0x22, 0x16, 0xFF));

            C2D_DrawRectSolid(START_X, y - 1.0f, 0.45f,
                              2.0f, CARD_H + 1.0f,
                              C2D_Color32(0x33, 0xCC, 0x55, 0xFF));
            ...
            {
                std::string sub;
                if (!nowPlayingTrack.empty())  sub  = nowPlayingTrack + "  ";
                if (!nowPlayingArtist.empty()) sub += nowPlayingArtist;
                if (sub.length() > 22) sub = sub.substr(0, 19) + "...";
                if (!sub.empty()) {
    ...
    if (virtIdx <= qSz + aSz && y < MAX_Y)
        drawStr("...", START_X + 4.0f, y, 0.5f,
                0.45f, 0.45f, C2D_Color32f(0.35f, 0.35f, 0.35f, 1.0f));
}
...
void renderLogOverlay() {
    ...
    // Header
    drawStr("Log  (SELECT to hide)", PAD + 6.0f, PAD + 4.0f, 0.92f,
            0.40f, 0.40f, C2D_Color32(0x44, 0x44, 0x44, 0xFF));

    if (lines.empty()) {
        drawStr("(empty)", PAD + 6.0f, PAD + 18.0f, 0.92f,
                0.40f, 0.40f, C2D_Color32f(0.3f, 0.3f, 0.3f, 1));
        return;
    }
    ...
    for (size_t i = 0; i < lines.size(); ++i) {
        ...
        if (y + LINE_H > MAX_Y) break;
        drawStr(lines[i].c_str(), TEXT_X, y, 0.92f,
                0.38f, 0.38f, C2D_Color32f(0.78f, 0.78f, 0.78f, 1));
    }
}
...
void renderBottomScreen(bool songPlaying, double positionSeconds,
    ...
    if (!songPlaying) {
        drawStr("No song playing", 4, 8, 0.5f,
